...
export function EditUserModal({ open, user, onClose }: EditUserModalProps) {
  ...
  const userDetailQuery = useUser(open && user ? user.id : null)
  ...
  const sourceUser = userDetailQuery.data ?? user

  useEffect(() => {
    if (!open || !sourceUser) return
    setForm(
  ...
  const supervisorOptions = useMemo(
    () =>
      (usersQuery.data ?? [])
        .filter((item) => item.id !== sourceUser?.id)
        .map((item) => ({
          value: String(item.id),
  ...
  const handleSave = (e?: FormEvent) => {
    ...
    const nextErrors = validateUpdateUserForm(form)
  ...
  return (
    <Modal
      ...
      description={sourceUser ? `${sourceUser.fullName} (${sourceUser.userName})` : undefined}
      size="lg"
      footer={
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <Button
            ...
            disabled={updateUser.isPending || !sourceUser || userDetailQuery.isLoading}
          >
            Kaydet
          </Button>
        </div>
    ...
    >
      {userDetailQuery.isLoading && !userDetailQuery.data ? (
        <p className="text-sm text-muted">Kullanıcı bilgileri yükleniyor…</p>
      ) : (
        <form
          id="edit-user-form"
          onSubmit={handleSave}
        ...
        >
          ...
          <section className="space-y-3">
            ...
            <Input
              ...
              hint="Boş bırakırsanız şifre değişmez."
          ...
          <section className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <SearchableSelect
                ...
                placeholder={
                  departmansQuery.isLoading
                    ? 'Departmanlar yükleniyor...'
                    : 'Departman ara veya seç...'
                }
